  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Customer conversations from the YIIVA app</p>
      </div>
          ...
          <div className="border-r border-gray-200 overflow-y-auto">
            {conversationsQuery.isPending ? (
              ...
              <div className="p-8 text-center">
                ...
                <p className="text-sm font-medium text-black mb-1">No messages yet</p>
    ...
    >
      <div className="flex items-center gap-3">
        {conversation.buyer.avatar ? (
          ...
          <span className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-sm font-semibold text-gray-600">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {conversation.unreadCount > 0 && (
              <span className="shrink-0 min-w-5 h-5 px-1.5 bg-black text-white rounded-full text-[11px] font-semibold flex items-center justify-center">
  ...
  return (
    <>
      ...
      <div className="px-6 py-4 border-b border-gray-200 flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-black">{conversation.buyer.name}</p>
          <p className="text-xs text-gray-500">Buyer on YIIVA</p>
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
        {historyQuery.isPending ? (
          <div className="space-y-3">
            {[0, 1, 2].map((i) => (
              <div key={i} className="h-10 animate-pulse rounded-lg bg-gray-100 w-2/3" />
            ))}
          </div>
        ) : (
                ...
                >
                  ...
                  <p
                    className={`text-[11px] mt-1 ${
                      mine ? 'text-gray-400' : 'text-gray-500'
                    } text-right`}
                  >
      ...
      <div className="px-4 py-3 border-t border-gray-200">
        <div className="flex items-end gap-2">
          <textarea
            ...
            rows={1}
            maxLength={2000}
            className="flex-1 resize-none px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:border-black text-sm max-h-32"
          />
          <button
            onClick={() => void handleSend()}
            disabled={!input.trim() || sending}
            className="p-2.5 bg-black text-white rounded-xl hover:bg-gray-800 transition-colors disabled:opacity-40"
          >
            <Send size={18} />
          </button>
        </div>
